...
public class SSC : Mod
{
    ...
    public override void HandlePacket(BinaryReader reader, int from)
    {
        ...
        switch ((MsgID)msg_id)
        {
            ...
            case MsgID.TrySave:
            {
                ...
                {
                    ChatHelper.SendChatMessageToClient(NetworkText.FromLiteral("name empty"), Color.Red, from);
                    return;
                }

                if (name.Length > 16)
                {
                    ChatHelper.SendChatMessageToClient(NetworkText.FromLiteral("name too long"), Color.Red, from);
                    return;
                }

                if (name.IndexOfAny(Path.GetInvalidFileNameChars()) != -1)
                {
                    ChatHelper.SendChatMessageToClient(NetworkText.FromLiteral("invalid  name"), Color.Red, from);
                    return;
                }
                ...
                if (first && Directory.GetFiles(dir, $"{name}.plr", SearchOption.AllDirectories).Length > 0) // 防止同名注册
                {
                    ChatHelper.SendChatMessageToClient(NetworkText.FromKey(Lang.mp[5].Key, name), Color.Red, from);
                    return;
                }

                if (!first && !File.Exists(Path.Combine(dir, id, $"{name}.plr")))
                {
                    Console.WriteLine("保存失败,文件不存在");
                    ChatHelper.DisplayMessageOnClient(NetworkText.FromLiteral("保存失败,文件不存在"), Color.Red, from);
                    return;
                }
            ...
            case MsgID.TryLoad:
            {
                if (Main.netMode == NetmodeID.Server)
                {
                    ...
                    if (Netplay.Clients.Where(c => c.IsActive).Any(c => Main.player[c.Id].name == name))
                    {
                        ChatHelper.SendChatMessageToClient(NetworkText.FromKey(Lang.mp[5].Key, name), Color.Red, from);
                        return;
                    }
            ...
            default:
            {
                throw new Exception($"invalid msg id {msg_id}");
            }
        }
    }
    ...
    public static void StreamPatcher(ModPacket binary, int toClient = -1, int ignoreClient = -1)
    {
        ...
        else
        {
            ...
            // XX-XX-XX-XX-Position->[XX-XX]-[FA]-[01]-01-09-08
            ...
            {
                var sp = ModContent.GetInstance<SSC>().GetPacket();
                sp.Write((byte)MsgID.Stream);
                sp.Write(false);
                sp.Write(false);
                sp.Write(length);
                sp.Write(io, 0, length);
                sp.Send(toClient, ignoreClient);
            }
